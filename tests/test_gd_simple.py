...
def test_batch_gd_multi_dimensional():
    ...
    X_test = np.array([1, 0, 0, 0, 1, 0, 0, 0, 1, 2, 5, 7]).reshape((4, 3)).astype(float)
    
    expected_closed = closed_form_solution(X, y)
    X_test_with_intercept = np.column_stack([np.ones(4), X_test])
    expected = X_test_with_intercept @ expected_closed
    
    model = LinearRegressionGD(learning_rate=0.01, num_iterations=2000, verbose=False)
    model.fit(X, y)
    actual = model.predict(X_test)
...
def test_batch_gd_loss_decreases():
    ...
    model = LinearRegressionGD(learning_rate=0.1, num_iterations=100, verbose=False)
    model.fit(X, y)
...
def test_sgd_multi_dimensional():
    ...
    X = np.array([1, 2, 3, 5, 4, 5, 4, 3, 3, 3, 2, 5, 1, 1, 2, 3]).reshape((4, 4)).astype(float)
    y = np.array([2, 5, 3, 8]).astype(float)
    ...
    model = LinearRegressionSGD(learning_rate=0.01, num_epochs=200, verbose=False)
    model.fit(X, y)
    actual = model.predict(X_test)
