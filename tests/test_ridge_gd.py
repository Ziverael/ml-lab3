...
def test_ridge_gd_one_dimensional():
    ...
    model = RidgeRegressionGD(
        alpha=alpha, learning_rate=0.1, num_iterations=2000, verbose=False
    )
...
def test_ridge_reduces_coefficient_magnitude():
    ...
    model_ols = RidgeRegressionGD(
        alpha=0, learning_rate=0.1, num_iterations=2000, verbose=False
    )
    model_ols.fit(X_scaled, y)

    # With regularization (alpha=1.0)
    model_ridge = RidgeRegressionGD(
        alpha=1.0, learning_rate=0.1, num_iterations=2000, verbose=False
    )
...
def test_ridge_handles_multicollinearity():
    ...
    model = RidgeRegressionGD(
        alpha=0.5, learning_rate=0.1, num_iterations=2000, verbose=False
    )
    model.fit(X, y)
...
def test_higher_alpha_more_regularization():
    ...
    model_weak = RidgeRegressionGD(
        alpha=0.1, learning_rate=0.1, num_iterations=2000, verbose=False
    )
    model_weak.fit(X_scaled, y)

    model_strong = RidgeRegressionGD(
        alpha=10.0, learning_rate=0.1, num_iterations=2000, verbose=False
    )
...
def test_ridge_intercept_not_regularized():
    ...
    y = (
        X @ np.array([1.5, -2.0]) + 5.0 + 0.1 * np.random.randn(50)
    )  # Intercept = 5.0

    # With very high regularization
    model = RidgeRegressionGD(
        alpha=100.0, learning_rate=0.1, num_iterations=2000, verbose=False
    )
    model.fit(X, y)
...
def test_ridge_loss_decreases():
    ...
    model = RidgeRegressionGD(
        alpha=0.5, learning_rate=0.1, num_iterations=500, verbose=False
    )
    model.fit(X, y)
...
def test_ridge_matches_sklearn_simple():
    ...
    # Our implementation
    model = RidgeRegressionGD(
        alpha=alpha, learning_rate=0.1, num_iterations=3000, verbose=False
    )
    model.fit(X, y)
