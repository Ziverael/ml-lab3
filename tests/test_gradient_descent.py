...
def test_batch_gd_simple_1d():
    ...
    model = LinearRegressionGD(learning_rate=0.1, num_iterations=1000, verbose=False)
    model.fit(X, y)
...
def test_batch_gd_multiple_features():
    ...
    model = LinearRegressionGD(learning_rate=0.1, num_iterations=2000, verbose=False)
...
def test_batch_gd_loss_decreases():
    ...
    model = LinearRegressionGD(learning_rate=0.1, num_iterations=500, verbose=False)
    model.fit(X_scaled, y)
    ...
    assert loss_history[-1] < loss_history[0], "Final loss should be less than initial loss"
    
    # Check that most consecutive pairs show decrease
    decreases = sum(loss_history[i+1] <= loss_history[i] for i in range(len(loss_history)-1))
    assert decreases > 0.9 * len(loss_history), "Loss should decrease in most iterations"
...
def test_batch_gd_convergence_tolerance():
    ...
    assert len(model.loss_history) < 10000, "Should converge before max iterations"
...
def test_learning_rate_too_large():
    ...
    model = LinearRegressionGD(learning_rate=10.0, num_iterations=100, verbose=False)
    model.fit(X, y)
    ...
    if not np.any(np.isnan(loss_history)) and not np.any(np.isinf(loss_history)):
...
def test_batch_gd_recovers_true_parameters():
    ...
    np.testing.assert_allclose(model.theta, expected_theta, rtol=1e-2, atol=1e-2)
...
def test_feature_scaling_improves_convergence():
    ...
    y = 2 * X_unscaled[:, 0] / 1000 + 5 * X_unscaled[:, 1] + 3 + np.random.randn(m)
...
def test_sgd_converges_to_similar_solution():
    ...
    # Batch GD
    model_batch = LinearRegressionGD(learning_rate=0.1, num_iterations=1000, verbose=False)
    model_batch.fit(X_scaled, y)
    
    # SGD
    model_sgd = LinearRegressionSGD(learning_rate=0.01, num_epochs=50, verbose=False)
    model_sgd.fit(X_scaled, y)
    
    # Parameters should be reasonably close
    np.testing.assert_allclose(model_sgd.theta, model_batch.theta, rtol=0.1, atol=0.1)
...
def test_sgd_prediction():
    ...
    model = LinearRegressionSGD(learning_rate=0.01, num_epochs=100, verbose=False)
    model.fit(X, y)
    predictions = model.predict(X_test)
...
def test_sgd_loss_general_decrease():
    ...
    model = LinearRegressionSGD(learning_rate=0.01, num_epochs=50, verbose=False)
    model.fit(X_scaled, y)
...
def test_all_methods_converge_similarly():
    ...
    np.testing.assert_allclose(model_batch.theta, model_sgd.theta, rtol=0.1, atol=0.1)
    np.testing.assert_allclose(model_batch.theta, model_minibatch.theta, rtol=0.05, atol=0.05)
